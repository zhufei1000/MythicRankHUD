...
local function GetEnglishDataText(value)
    value = SafeString(value)
    if not value or value == "" then return nil end
    -- International dungeon labels must never inherit localized client text.
    -- The English data currently uses ASCII; rejecting other bytes provides a
    -- strict fail-closed guarantee when a regional pack contains localized data.
    if value:find("[\128-\255]") then return nil end
    return value
end
...
local internationalDungeonSource
local internationalDungeonByMapID = {}

local function GetInternationalDungeonInfo(mapID)
    local region = GetRequiredDataPack()
    local API = _G.QFXMythicRankData
    local dungeons
    if type(API) == "table" and type(API.GetSeasonDungeons) == "function" and region then
        dungeons = SafeCall(API.GetSeasonDungeons, API, region)
    end
    if type(dungeons) ~= "table" then
        dungeons = nil
    end
    if dungeons ~= internationalDungeonSource then
        internationalDungeonSource = dungeons
        for key in pairs(internationalDungeonByMapID) do
            internationalDungeonByMapID[key] = nil
        end
        for _, dungeon in ipairs(dungeons or {}) do
            if type(dungeon) == "table" then
                local dungeonMapID = SafeNumber(dungeon.challengeModeID)
                if dungeonMapID then
                    internationalDungeonByMapID[dungeonMapID] = {
                        name = GetEnglishDataText(dungeon.name),
                        shortName = GetEnglishDataText(dungeon.shortName),
                    }
                end
            end
        end
    end
    local fallbackName, fallbackShortName
    if ns.GetFallbackEnglishDungeonInfo then
        fallbackName, fallbackShortName = SafeCall(ns.GetFallbackEnglishDungeonInfo, mapID)
        fallbackName = GetEnglishDataText(fallbackName)
        fallbackShortName = GetEnglishDataText(fallbackShortName)
    end
    local info = internationalDungeonByMapID[SafeNumber(mapID)]
    if info then
        return info.name or fallbackName, info.shortName or fallbackShortName
    end
    return fallbackName, fallbackShortName
end
...
local function IsAdapterEnabled(adapter)
    -- The native group finder is always a supported host, even while a custom
    -- group-finder addon is installed: however the player opens that window,
    -- the HUD must follow it. Priority (adapter order) only arbitrates between
    -- the mutually exclusive custom addons.
...
local function UpdateSeasonBar(refreshResources)
    ...
    for index = 1, 8 do
        ...
        if index <= count then
            ...
            card.name:SetText(GetMapAbbreviation(mapID))
...
local function UpdateWeeklyDetails()
    ...
    for index = 1, math.min(#maps, 8) do
        ...
        lower[#lower + 1] = string.format(
            "%s |cffffd100%s|r (%s)",
            TextureMarkup(texture), GetMapAbbreviation(mapID), table.concat(levels, " ")
        )
    end
...
local function CreateSidePanel()
    ...
    panel.vaultRows = {
        CreateVaultProgressRow(panel, L.VAULT_RAID or "Raid"),
        CreateVaultProgressRow(panel, L.VAULT_MYTHIC_PLUS or "Mythic+"),
        CreateVaultProgressRow(panel, L.VAULT_WORLD or "World"),
    }
    for index, row in ipairs(panel.vaultRows) do
...
local function UpdateVisibility(updates)
    ...
    -- Anchoring the overlays to a protected host such as PVEFrame makes the
    -- overlays protected too, so Show/Hide/SetPoint from addon code is blocked
    -- during combat (ADDON_ACTION_BLOCKED) even when nothing would change.
    -- Defer those mutations; the PLAYER_REGEN_ENABLED refresh applies them.
    -- Outside combat, skip the calls entirely unless the desired visibility
    -- actually changed: redundant SetShown on a hidden overlay is still a
    -- protected call on every game event.
...
local function HookHost(adapter, frame)
    ...
    frame:HookScript("OnHide", function(self)
        if integration.mainPanel == self then
            -- UpdateVisibility is the single source of truth for overlay
            -- visibility: reselecting the host applies hide/show/move in one
            -- place, so no direct Hide() here (a direct hide would desync the
            -- appliedShown bookkeeping and is a protected call besides).
...
-- Give load-on-demand boards a short window to create their frames. After the
-- window, normal addon/game events call HookAvailableHosts again; do not keep
-- a permanent polling timer alive for the entire play session.
...
eventFrame:SetScript("OnEvent", function(_, event, arg1, arg2, arg3, arg4)
    ...
    elseif event == "PLAYER_REGEN_ENABLED" then
        -- leaving combat: apply any visibility/layout mutations that combat
        -- lockdown deferred and refresh only the secure teleport bindings
        QueueRefresh(0.10, "teleports")
